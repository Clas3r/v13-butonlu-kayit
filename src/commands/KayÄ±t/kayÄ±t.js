...
30 Saniye Sonra Kayıt İptal Olacaktır.
    ...
    collector.on("collect", async (button) => {
      ...
      if(button.customId === "erkek") {
        ...
        const guild = message.guild
            ...
            const claser = client.users.cache.get(config.bot.owner);
        const claserkekmb = new MessageEmbed().setColor(message.member.displayHexColor).setAuthor({name: message.member.displayName, iconURL: author.avatarURL({ dynamic: true, size: 2048 })}).setFooter({text: (config.bot.BotFooter) , iconURL: claser.avatarURL({ dynamic: true, size: 2048 })})
        if (!names) {
            claserkekmb.setDescription(`Kullanıcının ismi \`${name} ${config.registration.symbol} ${age}\` olarak değiştirildi ve <@&${config.registration.oneman}> rolü verilerek kayıt edildi.`) 
        } else {
            claserkekmb.setDescription(`Kullanıcı başarıyla <@&${config.registration.oneman}> olarak kayıt edildi!\n\n Kullanıcının toplamda " ${names.length} " isim kayıtı görüntülendi.\n${names.map((data) => `${data}`).join("\n")}`) 
        }
...
msg.edit({
  ...
  if(button.customId === "kadin") {
    ...
    const guild = message.guild
        ...
        await guild.members.cache.get(member.id).roles.remove(config.registration.unregistered)
        const claser = client.users.cache.get(config.bot.owner);
    const claserkadinemb = new MessageEmbed().setColor(message.member.displayHexColor).setAuthor({name: message.member.displayName, iconURL: author.avatarURL({ dynamic: true, size: 2048 })}).setFooter({text: (config.bot.BotFooter) , iconURL: claser.avatarURL({ dynamic: true, size: 2048 })})
    if (!names) {
        claserkadinemb.setDescription(`Kullanıcının ismi \`${name} ${config.registration.symbol} ${age}\` olarak değiştirildi ve <@&${config.registration.onewoman}> rolü verilerek kayıt edildi.`) 
    } else {
        claserkadinemb.setDescription(`Kullanıcı başarıyla <@&${config.registration.onewoman}> olarak kayıt edildi!\n\n Kullanıcının toplamda " ${names.length} " isim kayıtı görüntülendi.\n${names.map((data) => `${data}`).join("\n")}`) 
    }
